      ...
      //   	'dom': 'Btip',
			...
			'columnDefs': [
				...
				},
				{
					...
					render: function(data, type, full, meta) {
						var status = {
							1: {'title': 'Office', 'class': ' badge-light-success'},
							2: {'title': 'Auditor', 'class': ' badge-light-info'},
							3: {'title': 'Trainer', 'class': ' badge-light-warning'},
						};
		...
		$('#submit').on('click', function(e) {
		    ...
		    var desc = $('#data-form  textarea[name="desc"]').val();
		    var password = $('#data-form  input[name="password"]').val();
		    var confPass = $('#data-form  input[name="confPass"]').val();
		    ...
		    table.row.add([username, name, position, status, createdDate, createdBy,  ""]).draw();
...
function doEdit(id) {
    ...
    $('#data-form input[name="name"]').val(table.row(id).data()[2]);
    $('#data-form input[name="username"]').val(table.row(id).data()[1]);

    $('#data-form select[name="positionId"]').val(table.row(id).data()[3]).trigger("change");

    if(table.row(id).data()[4] === "1")
